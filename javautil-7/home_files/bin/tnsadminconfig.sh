...
#! * tnsnames must be readable to anyone using sqlplus on the rdbms server
#!  the default permissions of 600 allow it to be used for tcp connections
#!  and for those in the dba account but no other sqlplus user can use
#! 
#! This script will add description for sales_reporting, the tutorial container
#!    sales_reporting=
#!      (DESCRIPTION =
...
* modify the hostnames in tnsnames.ora to be localhost and add  
    if [ $# -ne 1 ] ; then
