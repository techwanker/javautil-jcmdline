...
--
--This is based simply on gross revenue and the ratio is
--total sales for all the customers.
--
--Although this is easy to understand it is deficient, as for example
--one customer may be the only customer that sells this product.
-- 
--What is the normalized distribution of products by vending operators?
--For that we have to find the ratio of product by customer
...
where case_gtin = 
    ...
    group by etl_sale.ship_to_cust_id,
             etl_sale.case_gtin;
\
--Customer Product Percentage
--***************************
-- Will only show if sold
create or replace view customer_prod_pct_vw as
select  cp.ship_to_cust_id, 
        cp.case_gtin,
	sum(cp.tot_extended_net_amt) * 100 / ct.tot_extended_net_amt product_pct
from customer_product_12_vw cp,
     customer_12_vw ct
where cp.ship_to_cust_id = ct.ship_to_cust_id
group by cp.ship_to_cust_id, 
	cp.case_gtin,
	ct.tot_extended_net_amt
having ct.tot_extended_net_amt > 0;
     
select * from customer_prod_pct_vw 
order by ship_to_cust_id, 
	product_pct desc,
	case_gtin;
jjjj/*
select case_gtin, median(product_pct) median_product_pct
from customer_prod_pct_vw
group by case_gtin
order by median(product_pct) desc;
*/
create table product_stat
(
        case_gtin varchar(14),
	report_date date,
        tot_extended_net_ant numeric(14,2), 
	cust_count numeric(9),
        min_pct numeric(6,3),
        median_pct numeric(6,3),
        max_pct numeric(6,3),
        first_sale_dt date,
        slope numeric(5,5)
);
