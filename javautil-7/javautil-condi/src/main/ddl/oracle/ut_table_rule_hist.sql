...
create sequence ut_table_rule_hist_nbr_seq;
  ...
  CREATE TABLE UT_TABLE_RULE_HIST
   (	
	UT_TABLE_RULE_HIST_NBR NUMBER(9,0),
	...
	LOG_LVL NUMBER(1,0),
	 CONSTRAINT UT_TABLE_RULE_HIST_PK PRIMARY KEY (UT_TABLE_RULE_HIST_NBR) ENABLE,
	 CHECK ( severe_rule_flg in ('Y', 'N')) ENABLE,
	 CONSTRAINT CHECK_HIST_EXCEPTION_TYPE_CD CHECK (exception_type_cd IN ('I', 'E')) ENABLE,
	 CONSTRAINT UTR_CHECK_HIST_DISP_CD_ENUM CHECK ( DISP_CD_ENUM IN ('A', 'M', 'S')) ENABLE,
	 CONSTRAINT HIST_MAX_QUERY_SEC_CHK CHECK (max_query_sec > 1 and max_query_sec <= 120) ENABLE,
	 CONSTRAINT UTRH_UTR_FK FOREIGN KEY (UT_TABLE_RULE_NBR)
	  REFERENCES UT_TABLE_RULE (UT_TABLE_RULE_NBR) ENABLE,
	 CONSTRAINT UTRH_UDL_FK FOREIGN KEY (SCHEMA, TABLE_NAME)
	  REFERENCES UT_DATA_LOCATION (SCHEMA, TABLE_NAME) ENABLE,
	 CONSTRAINT UTRH_UTR_EXCLUDE_FK FOREIGN KEY (UT_TABLE_RULE_NBR_EXCLUDE)
	  REFERENCES UT_TABLE_RULE (UT_TABLE_RULE_NBR) ENABLE,
	 CONSTRAINT UTRH_UQ_FK FOREIGN KEY (UT_QUERY_NBR)
...
alter table ut_table_rule_hist add ut_area_resp_nbr number(9);

alter table ut_table_rule_hist add constraint utrh_uar_fk 
