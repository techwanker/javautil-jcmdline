...
is
  ...
  begin
    insert into logger_settings (    
      ...
      abort_stack_trace,          log_level,      msg_lvl
     ) values (
   	rec.logger_settings_id, rec.logger_token,   rec.process_name,
        ...
        rec.abort_stack_trace,   rec.log_level,      rec.msg_lvl
     );
    commit;
  ...
  function job_step_insert (
    ...
    stack_trace  in varchar default null
  ) return number
  is 
  ...
  begin
        ...
        if stack_trace is not null then
            stack := stack_trace;
        else
            stack := DBMS_UTILITY.format_call_stack ();
        end if;
