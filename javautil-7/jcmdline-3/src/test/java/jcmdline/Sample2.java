 ...
 * Sample2.java
 *
 * jcmdline Rel. @VERSION@ $Id: Sample2.java,v 1.2 2002/12/07 14:30:49 lglawrence Exp $
 *
 * Classes:
 *   public   Sample2
 *   
 ...
 * @version         jcmdline Rel. @VERSION@ $Id: Sample2.java,v 1.2 2002/12/07 14:30:49 lglawrence Exp $
 */
public class Sample2 {
    ...
    private static final Parameter[] opts = new Parameter[] {
        ...
        new FileParam("file2",
                      ...
                      FileParam.IS_DIR),
        new FileParam("file3",
                      "an optional, hidden file parameter",
    ...
    private static final String helpText =
        "This command is used to copy an html file, old_file, to a new file, " +
        "new_file.\n\n" +
    ...
    /**
     * main - doesn't actually do anything but process command line 
     * paramters and print them out.
     *
     * @param args          command line arguments
     */
    public static void main(String[] args) {
        /*
        CmdLineParser cl = new CmdLineParser(
                "Sample2",
                "a class to test command line parameters",
                "V 5.2",
                helpText,
                opts,
                arguments);
        */
        ...
        FileParam newfile = (FileParam) cl.getArg("new_file");
        if (! oldfile.getFile().getPath().endsWith(".html")) {
            cl.exitUsageError("<old_file> must end with '.html'");
        }

        Collection nopts = cl.getOptions();
        Collection nargs = cl.getArgs();
        Collection vals;
        Parameter p;
        String valString;
        System.out.println("\n\nArgs:\n");
        for (Iterator itr = nargs.iterator(); itr.hasNext(); ) {
        ...
        System.out.println("\n\nOptions:\n");
        for (Iterator itr = nopts.iterator(); itr.hasNext(); ) {
            p = (Parameter) itr.next();
            vals = p.getValues();
            valString = "";
            for (Iterator itr2 = vals.iterator(); itr2.hasNext(); ) {
