 ...
 */
@Deprecated
public class CrosstabbedWorksheetHelper extends BaseWorksheetRenderer {
	...
	public CrosstabbedWorksheetHelper(final HSSFSheet sheet, final HSSFCellStyleFactory styleFactory) {
	 ...
	 */
	public void emitCrosstabHeaders(final List<ColumnMetadata> columns, final Style baseStyle, final int startingRow,
	 ...
	 */
	public void addGroupHeaderSpanCell(final ColumnMetadata column, final Style baseStyle, final int rowIndex,
	...
	public void addCell(final ColumnMetadata column, final Style baseStyle, final int rowIndex, final int columnIndex) {
		if (logger.isDebugEnabled()) {
		...
		cellStyle = getStyle(baseStyle, null, column.getHorizontalAlignment());
		String heading = getMergedColumnHeading(column);

		if (heading.indexOf('\n') > -1) {
			cellStyle = addWordWrap(cellStyle);
		}
		super.addCell(rowIndex, columnIndex, heading, baseStyle, column);
		if (column.getColumnDisplaySize() != null) {
			setColumnWidth(columnIndex, column.getColumnDisplaySize().intValue());
		}
	}
	 ...
	 * @param metadataByColumnName
	 */
	public void emitHeadersForRowIdentifiers(final Dataset dataset, final Style baseStyle, final int startingRow,
		...
		for (final int rowidColumnIndex : sourceRowidColumnIndexes) {
			...
			column = dataset.getMetadata().getColumnMetaData(rowidColumnIndex);
			if (logger.isDebugEnabled()) {
				logger.debug("adding rowIdentifier header at " + rowIndex + "," + columnIndex + " " + column);
			}
