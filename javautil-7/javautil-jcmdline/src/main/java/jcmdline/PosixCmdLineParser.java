...
public class PosixCmdLineParser implements CmdLineParser {
     ...
     *
     * @param clargs command line arguments passed to main
     * @param opts options by optionname
     * @param args 
    ...
    public void parse(String[] clargs, Map opts, List args) 
    	...
    	logger.info("clargs {}\nopts {)\nargs {}",clargs,opts,args);
        if (clargs == null) {
            clargs = new String[] {};
        }
        int i = processOptions(clargs, opts);
        logger.info("i {} number of paremeters used by options",i);
     ...
     private int processOptions(String[] params, Map options) 
        ...
        for (i = 0; i < params.length; i++) {
            ...
            if (params[i].equals("--")) {  // end of options
                ...
                p.addValue(val);
                logger.info("adding p {} val {}",p,val);
            } else {
                break;      // end of options
            }
        }
        logger.info("used up {} arguments",i);
        return i;
    }
     ...
     *                      is to start this is equal to the number of arguments consumed in processing
     *                      options
     * @param params        the command line parameters
     * @param args          The variable to hold all non option arguments
     * @throws              CmdLineException if any processing errors are 
     *                      encountered.
     */
    private void processArguments(int stIdx, String[] params, List args) 
            throws CmdLineException {
    	logger.info("stIdx {}\nparams {},\nargs{}", stIdx, params, args);
        int argIdx = 0;
        Parameter p;
        for (int pIdx = stIdx; pIdx < params.length; pIdx++) {
            if (argIdx >= args.size()) {
            	logger.error("argIdx {} >= args.size() {}",argIdx,args.size());
                throw new CmdLineException(
