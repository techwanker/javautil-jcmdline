...
public class ParameterCreator {
	...
	public Parameter[] generateParameterArgumentsForClass(
			final Class<? extends Object> clazz)
					throws IllegalArgumentException {
		if (logger.isDebugEnabled()) {
			logger.debug("generating parameter arguments for " + clazz.getName());
		}
		final Field[] fields = clazz.getDeclaredFields();
		final  ArrayList<Parameter> ret = new ArrayList<Parameter>();
		for (final Field field : fields) {
			final boolean hasArguments = field.getAnnotation(Argument.class) != null;
		...
		Parameter[] arguments = new Parameter[ret.size()];
		int x = 0;
		for (Parameter p :  ret) {
			arguments [x++] = p;
		}
	...
	private void showArguments(Parameter[] arguments) {
		...
		for (Parameter p : arguments) {
			...
			if (p instanceof StringParam) {
				...
				//				Collection<Object> vees  = sp.getValues();
				for (String o : sp.getValues()) {
					logger.info("o {}",o);
				}
			}
		}
