 ...
 * @version         jcmdline Rel. @VERSION@ $Id: AbstractParameter_UT.java,v 1.2 2002/12/07 14:30:49 lglawrence Exp $
 */
public class AbstractParameter_UT extends BetterTestCase {
     ...
     */
    public AbstractParameter_UT(String name) {
        super(name);
    }
    ...
    public void testDesc() {
        ...
        p.setDesc(s);
        assertEquals("getDesc() returns wrong string",
                     s, p.getDesc());
    }
    ...
    public void testAddValueSingle() {
        ...
        assertEquals("addValue() did not store first value correctly",
                     val1, p.getValue());
        ArrayList vals = new ArrayList(p.getValues());
        assertEquals("getValues() returned wrong nbr of values after first add",
                     1, vals.size());
        assertEquals("getValues() did not store first value correctly",
                     val1, vals.get(0));
    ...
    public void testAddValueMulti() {
        ...
        assertEquals("addValue() did not store first value correctly",
                     val1, p.getValue());
        ArrayList vals = new ArrayList(p.getValues());
        assertEquals("getValues() returned wrong nbr of values after adds",
                     2, vals.size());
        assertEquals("getValues() did not store first value correctly",
                     val1, vals.get(0));
        assertEquals("getValues() did not store second value correctly",
                     val2, vals.get(1));
        assertTrue("isSet() returns false after value set",
               p.isSet());
    }
    ...
    public void testSetValuesCollection() throws CmdLineException {
        ...
        vals.add("val2");
        p.setValues(vals);
        ArrayList currVals = new ArrayList(p.getValues());
        assertEquals("Parameter does not contain the right # of values",
                     2, currVals.size());
        assertTrue("Parameter does not contain val1",
               currVals.contains("val1"));
    ...
    public void testSetValue() throws CmdLineException {
        ...
        p.setValue(v);
        ArrayList vals = new ArrayList(p.getValues());
        assertEquals("wrong # values after first set of value",
                     1, vals.size());
        assertEquals("Wrong value set after first try",
                     v, (String)vals.get(0));

        v = "new value";
        p.setValue(v);
        vals = new ArrayList(p.getValues());
        assertEquals("wrong # values after second set of value",
                     1, vals.size());
        assertEquals("Wrong value set after second try",
                     v, (String)vals.get(0));
    }
    ...
    public static void main (String[] args) {
        doMain(args, AbstractParameter_UT.class);
    }
     ...
     * @version         jcmdline Rel. @VERSION@ $Id: AbstractParameter_UT.java,v 1.2 2002/12/07 14:30:49 lglawrence Exp $
     */
    class UnitTestParam extends AbstractParameter {
