...
public abstract class BetterTestCase extends TestCase {
    ...
    /**
     * the beginning of the string returned when a key is not defined
     * in the strings.properties resource file
     */
    private static final String missingMsg = 
     ...
     *    }
     * </code>
     * Should this method be used in setUp(), a complementary call should 
     * appear in tearDown:
     *
     ...
     *                      line of desc is indented on next line.
    ...
    /**
     * A convenience method that returns the base portion (that which
     * follows the last '.', or '$' in case of a nested class) of this
     * object's class name.
     *
     ...
     * to accept debug levels and an optional list of test names.  If no
     * test names are passed as arguments, all tests in the class will be run.
     * <P>
     * The '-debug' command line option sets the root logging level to the
     * corresponding level, where 1 = FINEST ... 7 = SEVERE.  If not specified,
     * 7 (SEVERE) is the default.  A ConsoleHandler is installed so that
     * log records of the appropriate level will be written to stderr.
     * <P>
     * This method is intended to be called from a subclass's main()
     * method as follows (for class JobAssignment_UT):
     * <pre>
     ...
     * @param c             a class object representing the implementing class
     */
    protected static void doMain(String[] args, Class c) {
        String cName = baseClassName(c.getName());
        String classUnderTest = cName.endsWith("_UT")
            ? cName.substring(0, cName.length() - 3)
            : cName;
        ...
        StringParam pTests = new StringParam(
                "test",
        ...
        helpText.append("Runs the unit test for class ")
                .append(classUnderTest)
                .append(".\n\n")
                .append("If desired, specific tests can be run by specifying ")
        ...
        helpText.append("\n\nSpecification of '-log' will generate more ")
                ...
                .append("Log messages will ")
                .append("be written to stdout.  The unit tests themselves ")
                .append("log trace type information at log level FINE.");
        LoggerCmdLineHandler clp = 
            new LoggerCmdLineHandler(System.out,
            new HelpCmdLineHandler(helpText.toString(),
            new VersionCmdLineHandler("jcmdline Rel. @VERSION@ $Id: BetterTestCase.java,v 1.2 2002/12/07 14:30:49 lglawrence Exp $",
                cName,
                "tests class " + classUnderTest,
                new Parameter[] {},
                new Parameter[] { pTests })));
     ...
     * @version         jcmdline Rel. @VERSION@ $Id: BetterTestCase.java,v 1.2 2002/12/07 14:30:49 lglawrence Exp $
     * @see             Image
     */
