 ...
 * StringParam_UT.java
 *
 * jcmdline Rel. @VERSION@ $Id: StringParam_UT.java,v 1.3 2005/02/06 23:41:14 lglawrence Exp $
 *
 * Classes:
 ...
 * @version         jcmdline Rel. @VERSION@ $Id: StringParam_UT.java,v 1.3 2005/02/06 23:41:14 lglawrence Exp $
 */
public class StringParam_UT extends BetterTestCase {
     ...
     */
    public StringParam_UT(String name) {
        super(name);
    }
    ...
    public void testCtor3() {
        ...
        assertEquals("tag set wrong",
                     tag, sp.getTag());
        assertEquals("desc set wrong",
                     desc, sp.getDesc());
    ...
    public void testCtor4() {
        ...
        int max = 100;
        StringParam sp = 
            new StringParam(tag, desc, min, max, StringParam.OPTIONAL);
        assertEquals("tag set wrong", tag, sp.getTag());
    ...
    public void testCtor7() {
        StringParam sp = new StringParam(tag, desc, acceptVals);
        assertEquals("tag set wrong", tag, sp.getTag());
        assertEquals("desc set wrong", desc, sp.getDesc());
    ...
    public void testCtor8() {
        StringParam sp = 
            new StringParam(tag, desc, acceptVals, StringParam.OPTIONAL);
        assertEquals("tag set wrong", tag, sp.getTag());
        assertEquals("desc set wrong", desc, sp.getDesc());
        assertTrue("optional flag not set to true", sp.isOptional());
        assertEquals("acceptableValues set wrong",
    ...
    public void testCtor10() {
        StringParam sp = 
            new StringParam(tag, desc, acceptVals, 
                            StringParam.OPTIONAL, 
                            StringParam.MULTI_VALUED, 
                            StringParam.HIDDEN);
        assertEquals("tag set wrong", tag, sp.getTag());
     ...
     * return 0 and StringParam.UNSPECIFIED_LENGTH, respectively,
     * if they had not been set.
     */
    public void testMinMaxUnspec() {
        StringParam sp = new StringParam("mytag", "mydesc");
        int len = sp.getMinValLen();
        assertEquals(
            "getMinValLen() did not return UNSPECIFIED_LENGTH when not set",
            0, sp.getMinValLen());
        assertEquals(
     ...
     *  <li>setMaxValLen() throws an IllegalArgumentException when value set 
     *  less than min.</li>
     *  <li>setMaxValLen() throws an IllegalArgumentException when value 
     *  is negative.</li>
     * </ul>
     */
    public void testSetMaxValLen() {
        StringParam sp = new StringParam("mytag", "mydesc", 2, 10, 
                                         ...
                                         StringParam.MULTI_VALUED);
        sp.setMaxValLen(5);
        assertEquals("Max value not set correctly after call",
                     5, sp.getMaxValLen());
        sp.setMaxValLen(2);
        assertEquals("Max value not set correctly when set same as min",
                     2, sp.getMaxValLen());
        try {
            sp.setMaxValLen(1);
     ...
     *  greater than max.</li>
     *  <li>setMinValLen() throws an IllegalArgumentException when value 
     *  is negative.</li>
     * </ul>
     */
    public void testSetMinValLen() {
        StringParam sp = new StringParam("mytag", "mydesc", 2, 10, 
                                         ...
                                         StringParam.MULTI_VALUED);
        sp.setMinValLen(5);
        assertEquals("Min value not set correctly after call",
                     5, sp.getMinValLen());
        sp.setMinValLen(10);
        assertEquals("Min value not set correctly when set same as max",
                     10, sp.getMinValLen());
        try {
            sp.setMinValLen(11);
    ...
    public void testMultiValued() throws CmdLineException {
        ...
        sp.addValue(s1);
        sp.addValue(s2);
        Collection vals = sp.getValues();
        Iterator itr = vals.iterator();
        assertEquals("Multi-valued StringParam contains wrong # of values",
                     2, vals.size());
        assertEquals("First value is not correct",
                     s1, itr.next());
        assertEquals("Second value is not correct",
                     s2, itr.next());
    }
    ...
    public void testAcceptableValues() throws CmdLineException {
        ...
        StringParam sp = new StringParam("myTag", "myDesc", 
                new String[] { "val1", "val2" }, StringParam.OPTIONAL, 
                               StringParam.MULTI_VALUED);
        sp.addValue("val1");
        sp.addValue("val2");
        try {
        ...
        Collection vals = sp.getValues();
        assertEquals("StringParam contains wrong # of values",
                     2, vals.size());
    }
    ...
    public static void main (String[] args) {
        doMain(args, StringParam_UT.class);
    }
}
