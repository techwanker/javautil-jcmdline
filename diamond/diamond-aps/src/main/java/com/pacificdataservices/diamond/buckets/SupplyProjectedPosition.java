 ...
 */
public class SupplyProjectedPosition  {
	...
	public SupplyProjectedPosition(Collection<Supply> supplies, DateGenerator dateGenerator) {
	...
	private void generateDetailedProjectedPosition() {
		...
		for (Supply supply : supplies) {
			logger.info("i = " + i + " supply: " + supply.getIdentifierString());
			...
			for (Allocation allocation : allocations) {
				Date firstBucketDate = getFirstBucketDate(supplyHmmBuckets, allocation);
				...
				if (logDetail) {
					...
					if (trace) {
						logger.info("adjusting {} values starting with key {} for availDt {} needBy {}", keyCount, sdf.format(firstKey), sdf.format(supply.getAvailDate()), 
			...
			for (String itemCd : supply.getIcItemMastByItemCd().keySet()) {
				...
				// same one for multiple parts
				logger.info("Putting ==> " + supply.getSupplyIdentifier() + " "  + id.format() + " " + supplyHmmBuckets);
	...
	public Date getFirstBucketDate(SupplyBuckets buckets, Allocation allocation) {
		...
		if (trace) {
			logger.info("Supply id {} availDate {} needBy: {} allocDate: {} firstBucketDate {}",
	...
	public MultiKeyHashMapOfLists<SupplyBuckets> groupBy(int... indicies) {
		...
		logger.info("groupBy grouping bucket count" + supplyBucketsMap.size() + " with " + indexSB.toString());
		...
		if (trace) {
			logger.info("groupBy into " + grouped.size() + " groups");
...
//		for (Entry<MultiKey,SupplyBuckets> e: supplyBucketsMap.entrySet()) {
//			sb.append(e.getValue().format(e.getKey()));
//			sb.append("\n");
//		}
//		return sb.toString();
//	}
