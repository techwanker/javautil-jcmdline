...
--     trunc(h.ORD_DT,'MM') ord_dt_mm,
     date_trunc('month',h.ORD_DT) ord_dt_mm,
...
CREATE OR REPLACE VIEW aps_avail_replen (
  sply_identifier, item_nbr,    facility,        aps_sply_sub_pool_nbr, 
  org_nbr_mfr,     org_nbr_vnd, avail_dt,        gross_avail_qty, 
  replen_um,       rev_lvl,     po_line_dtl_nbr, unit_cost_stk_um, 
  cntry_cd_origin, lot_manufacture_after_dt, lot_not_expire_before_dt, aps_avail_dt, 
  po_line_hdr_nbr) AS 
SELECT  
  poh.po_cd || '-' || lpad(plh.po_line_nbr::varchar,3,'0') || '-' || lpad(pld.po_line_dtl_nbr::varchar,6,'0') sply_identifier,
  plh.item_nbr,
  pld.facility,
